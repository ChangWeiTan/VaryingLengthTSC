...
/**
 * This is a class  generate prefix time series
 *
 * @author Chang Wei
 */
public class Prefix extends DataGenerator {
    private final Random random = new Random();

    private Prefix(final long seed) {
        random.setSeed(seed);
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(String.format("[PREFIX] Output path:    %s", outputPath));
        System.out.println(String.format("[PREFIX] Dataset path:   %s", datasetPath));
        System.out.println(String.format("[PREFIX] Problem:        %s", problem));
        System.out.println(String.format("[PREFIX] Percentage:     %s", percentageTransformed));
        System.out.println(String.format("[PREFIX] Seed:           %s", seed));

        outputPath += "UCRArchive_2018_Prefix/" + problem;

        Path.setOutputPath(outputPath);
        Path.setDatasetPath(datasetPath);

        final Prefix dataTransformer = new Prefix(seed);
        ...
        System.out.println("[PREFIX] Transforming data");
        ...
        System.out.println("[PREFIX] Saving data");
    ...
    public Sequences transform(final Sequences data, final double percentageTransformed) {
        ...
        for (int i = 0; i < numberTransformed; i++) {
            ...
            System.arraycopy(ts, 0, newTs, 0, newLength);
