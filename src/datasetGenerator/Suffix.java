...
/**
 * This is a class  generate suffix time series
 *
 * @author Chang Wei
 */
public class Suffix extends DataGenerator {
    private final Random random = new Random();

    private Suffix(final long seed) {
        random.setSeed(seed);
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(String.format("[SUFFIX] Output path:    %s", outputPath));
        System.out.println(String.format("[SUFFIX] Dataset path:   %s", datasetPath));
        System.out.println(String.format("[SUFFIX] Problem:        %s", problem));
        System.out.println(String.format("[SUFFIX] Percentage:     %s", percentageTransformed));
        System.out.println(String.format("[SUFFIX] Seed:           %s", seed));

        outputPath += "UCRArchive_2018_Suffix/" + problem;

        Path.setOutputPath(outputPath);
        Path.setDatasetPath(datasetPath);

        final Suffix dataTransformer = new Suffix(seed);
        ...
        System.out.println("[SUFFIX] Transforming data");
        ...
        System.out.println("[SUFFIX] Saving data");
    ...
    public Sequences transform(final Sequences data, final double percentageTransformed) {
        ...
        for (int i = 0; i < numberTransformed; i++) {
            ...
            final int startIdx = candidateLength - newLength;
